...
var decodeHTML4Entities = function(input, strict, parseError) {
    ...
    for (var i=1; i<segments.length; i++) {
        ...
        if (seg.charAt(0) == '#') {
            ...
            if (isEmpty) {
                parseError("missing digit in numeric character reference",7);
                output += '&' + seg;
                continue;
            }
            if (cc == 59) {
            ...
            if ( (code > 1114111) ) {
                ...
                if ( ((code > 64975) && (code < 65008)) ||
                      ([65535,131070,131071,196606,196607,262142,262143,327678,327679,393214,393215,458750,458751,524286,524287,589822,589823,655358,655359,720894,720895,786430,786431,851966,851967,917502,917503,983038,983039,1048574,1048575,1114110,1114111].indexOf(code) >= 0) ) {
                    ...
                    parseError("control character character reference",4);
                    var k = [128,130,131,132,133,134,135,136,137,138,139,140,142,145,146,147,148,149,150,151,152,153,154,155,156,158,159].indexOf(code);
                    if (k >= 0) {
                        code = [8364,8218,402,8222,8230,8224,8225,710,8240,352,8249,338,381,8216,8217,8220,8221,8226,8211,8212,732,8482,353,8250,339,382,376][k];
                    }
                }

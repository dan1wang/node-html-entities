...
var decodeHTML4Entities = function(input, semiRequired, allowAllCap) {
    ...
    for (i=1; i<segments.length; i++) {
        ...
        if (segment.length > 8) {
            trailingSemiColon = input.charAt(9) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,8);
                if (subStr == "thetasym") {
                    i += trailingSemiColon?10:9;
                    output += "\u03D1"
                              + segment.substring(trailingSemiColon?9:8, segment.length - 1);
                    continue;
                }
                if (allowAllCap && (subStr == "thetasym")) {
                    i += trailingSemiColon?9:8;
                    output += "\u03D1"
                              + segment.substring(trailingSemiColon?9:8, segment.length - 1);
                    continue;
                }
            ...
            trailingSemiColon = input.charAt(8) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,7);
                j = ["alefsym","Epsilon","epsilon","omicron","Omicron","upsilon","Upsilon"].indexOf(subStr);
                if (j >= 0) {
                    output += ["\u2135","\u0395","\u03B5","\u03BF","\u039F","\u03C5","\u03A5"][j]
                              + segment.substring(trailingSemiColon?8:7, segment.length - 1);
                    continue;
                }
                if (allowAllCap && (subStr == "alefsym")) {
                    i += trailingSemiColon?8:7;
                    output += "\u2135"
                              + segment.substring(trailingSemiColon?8:7, segment.length - 1);
                    continue;
                }
            ...
            if (!semiRequired || trailingSemiColon) {
                ...
                if (allowAllCap) {
                    j = ["brvbar","curren","divide","forall","frac12","frac14","frac34","hearts","hellip","iquest","lfloor","lowast","lsaquo","middot","otimes","permil","plusmn","rfloor","rsaquo","sigmaf","spades","there4","thinsp","weierp"].indexOf(subStr);
                    if (j >= 0) {
            ...
            trailingSemiColon = input.charAt(6) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,5);
                ...
                if (allowAllCap) {
                    j = ["acute","asymp","bdquo","cedil","clubs","crarr","diams","empty","equiv","exist","frasl","iexcl","image","infin","laquo","lceil","ldquo","lsquo","mdash","micro","minus","nabla","ndash","notin","oline","oplus","pound","radic","raquo","rceil","rdquo","rsquo","sbquo","szlig","tilde","times","trade","upsih"].indexOf(subStr);
                    if (j >= 0) {
            ...
            trailingSemiColon = input.charAt(5) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,4);
                j = ["apos","auml","Auml","beta","Beta","bull","cent","circ","cong","copy","COPY","dArr","darr","emsp","ensp","Euml","euml","euro","fnof","hArr","harr","iota","Iota","isin","Iuml","iuml","lang","larr","lArr","macr","nbsp","nsub","ordf","ordm","ouml","Ouml","para","part","perp","prod","prop","quot","QUOT","rang","rArr","rarr","real","sdot","sect","sube","sup1","sup2","sup3","supe","uArr","uarr","uuml","Uuml","yuml","Yuml","zeta","Zeta","zwnj"].indexOf(subStr);
                if (j >= 0) {
                ...
                if (allowAllCap) {
                    j = ["apos","bull","cent","circ","cong","emsp","ensp","euro","fnof","isin","lang","macr","nbsp","nsub","ordf","ordm","para","part","perp","prod","prop","rang","real","sdot","sect","sube","sup1","sup2","sup3","supe","zwnj"].indexOf(subStr);
                    if (j >= 0) {
            ...
            trailingSemiColon = input.charAt(4) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,3);
                j = ["amp","AMP","and","ang","cap","chi","Chi","cup","deg","eta","Eta","eth","ETH","int","loz","lrm","not","Phi","phi","piv","psi","Psi","reg","REG","rho","Rho","rlm","shy","sim","sub","sum","sup","Tau","tau","uml","yen","zwj"].indexOf(subStr);
                if (j >= 0) {
                ...
                if (allowAllCap) {
                    j = ["and","ang","cap","cup","deg","int","loz","lrm","not","piv","rlm","shy","sim","sub","sum","sup","uml","yen","zwj"].indexOf(subStr);
                    if (j >= 0) {
            ...
            trailingSemiColon = input.charAt(3) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,2);
                j = ["ge","GT","gt","le","LT","lt","mu","Mu","ne","ni","nu","Nu","or","pi","Pi","Xi","xi"].indexOf(subStr);
                if (j >= 0) {
                ...
                if (allowAllCap) {
                    j = ["ge","le","ne","ni","or"].indexOf(subStr);
                    if (j >= 0) {
                        output += ["\u2265","\u2264","\u2260","\u220B","\u2228"][j]
                                  + segment.substring(trailingSemiColon?3:2, segment.length - 1);
                        continue;
...
var decodeHTML5Entities = function(input, semiRequired, allowAllCap) {
    ...
    for (i=1; i<segments.length; i++) {
        ...
        if (segment.length > 33) {
            trailingSemiColon = input.charAt(34) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,33);
                if (subStr == "CounterClockwiseContourIntegra") {
                    i += trailingSemiColon?35:34;
                    output += "\u2233"
            ...
            trailingSemiColon = input.charAt(27) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,26);
                if (subStr == "ClockwiseContourIntegra") {
                    output += "\u2232"
                              + segment.substring(trailingSemiColon?27:26, segment.length - 1);
                    continue;
                } else if (subStr == "DoubleLongLeftRightArro") {
                    output += "\u27FA"
                              + segment.substring(trailingSemiColon?27:26, segment.length - 1);
                    continue;
                }
            ...
            trailingSemiColon = input.charAt(26) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,25);
                if (subStr == "NotNestedGreaterGreate") {
                    i += trailingSemiColon?27:26;
                    output += "\u2AA2\u0338"
            ...
            trailingSemiColon = input.charAt(25) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,24);
                if (subStr == "DiacriticalDoubleAcut") {
                    output += "\u02DD"
                              + segment.substring(trailingSemiColon?25:24, segment.length - 1);
                    continue;
                } else if (subStr == "NotSquareSupersetEqua") {
                    output += "\u22E3"
                              + segment.substring(trailingSemiColon?25:24, segment.length - 1);
                    continue;
                }
            ...
            trailingSemiColon = input.charAt(24) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,23);
                j = ["CloseCurlyDoubleQuot","DoubleContourIntegra","FilledVerySmallSquar","NegativeVeryThinSpac","NotPrecedesSlantEqua","NotRightTriangleEqua","NotSucceedsSlantEqua"].indexOf(subStr);
                if (j >= 0) {
                    output += ["\u201D","\u222F","\u25AA","\u200B","\u22E0","\u22ED","\u22E1"][j]
            ...
            trailingSemiColon = input.charAt(23) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,22);
                j = ["CapitalDifferential","DoubleLeftRightArro","DoubleLongRightArro","EmptyVerySmallSquar","NestedGreaterGreate","NotDoubleVerticalBa","NotGreaterSlantEqua","NotLeftTriangleEqua","NotSquareSubsetEqua","OpenCurlyDoubleQuot","ReverseUpEquilibriu"].indexOf(subStr);
                if (j >= 0) {
                    output += ["\u2145","\u21D4","\u27F9","\u25AB","\u226B","\u2226","\u2A7E\u0338","\u22EC","\u22E2","\u201C","\u296F"][j]
            ...
            trailingSemiColon = input.charAt(22) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,21);
                j = ["DoubleLongLeftArro","DownLeftRightVecto","LeftArrowRightArro","leftrightsquigarro","NegativeMediumSpac","NotGreaterFullEqua","NotRightTriangleBa","RightArrowLeftArro","SquareSupersetEqua"].indexOf(subStr);
                if (j >= 0) {
                    output += ["\u27F8","\u2950","\u21C6","\u21AD","\u200B","\u2267\u0338","\u29D0\u0338","\u21C4","\u2292"][j]
            ...
            trailingSemiColon = input.charAt(19) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,18);
                j = ["circlearrowrigh","DiacriticalAcut","DiacriticalGrav","DiacriticalTild","DoubleRightArro","DownArrowUpArro","downharpoonrigh","EmptySmallSquar","GreaterEqualLes","GreaterFullEqua","LeftAngleBracke","LeftUpDownVecto","LessEqualGreate","NonBreakingSpac","NotPrecedesEqua","NotRightTriangl","NotSucceedsEqua","NotSucceedsTild","NotSupersetEqua","ntrianglerighte","rightharpoondow","rightrightarrow","RightTriangleBa","RightUpTeeVecto","RightUpVectorBa","twoheadleftarro","UnderParenthesi","UpArrowDownArro","vartrianglerigh"].indexOf(subStr);
                if (j >= 0) {
            ...
            trailingSemiColon = input.charAt(18) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,17);
                j = ["bigtriangledow","circlearrowlef","CloseCurlyQuot","ContourIntegra","curvearrowrigh","DoubleDownArro","DoubleLeftArro","downharpoonlef","DownRightVecto","leftharpoondow","leftrightarrow","LeftRightVecto","LeftTriangleBa","LeftUpTeeVecto","LeftUpVectorBa","LowerRightArro","nleftrightarro","nLeftrightarro","NotGreaterEqua","NotGreaterTild","NotHumpDownHum","NotLeftTriangl","NotSquareSubse","ntrianglelefte","OverParenthesi","RightDownVecto","rightleftarrow","rightsquigarro","rightthreetime","ShortRightArro","straightepsilo","trianglerighte","UpperRightArro","vartrianglelef"].indexOf(subStr);
                if (j >= 0) {
                    output += ["\u25BD","\u21BA","\u2019","\u222E","\u21B7","\u21D3","\u21D0","\u21C3","\u21C1","\u21BD","\u21C6","\u294E","\u29CF","\u2960","\u2958","\u2198","\u21AE","\u21CE","\u2271","\u2275","\u224E\u0338","\u22EA","\u228F\u0338","\u22EC","\u23DC","\u21C2","\u21C4","\u219D","\u22CC","\u2192","\u03F5","\u22B5","\u2197","\u22B2"][j]
            ...
            trailingSemiColon = input.charAt(12) == ';';
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,11);
                j = ["backprim","backsime","Backslas","bigotime","centerdo","CenterDo","checkmar","CircleDo","complexe","Congruen","Coproduc","dotsquar","DoubleDo","downarro","DownArro","Downarro","DownBrev","gtrappro","gtreqles","gvertneq","heartsui","HumpEqua","leftarro","LeftArro","Leftarro","LeftFloo","lesseqgt","LessTild","lvertneq","Mellintr","MinusPlu","ngeqslan","nleqslan","NotCupCa","NotExist","NotSubse","nparalle","nshortmi","nsubsete","nsupsete","OverBrac","pitchfor","PlusMinu","rational","spadesui","subseteq","subsetne","supseteq","supsetne","therefor","Therefor","ThinSpac","triangle","TripleDo","UnionPlu","varpropt"].indexOf(subStr);
                if (j >= 0) {
            ...
            if (!semiRequired || trailingSemiColon) {
                subStr = input.substring(0,4);
                j = ["a","a","a","D","d","e","e","e","g","g","g","G","g","g","G","G","i","i","I","i","i","l","l","l","l","L","l","L","L","m","M","m","n","n","N","n","O","o","o","P","p","p","P","p","R","r","S","s","w","w","X","x"].indexOf(subStr);
                if (j >= 0) {
...
var decodeHTML4EntitiesStrict = function(input) {
    ...
    while (i < len) {
        ...
        if (char == '&') {
            if (i + 3 < len) {
                if (input.charAt(i+3) == ';') {
                    subStr = input.substring(i+1,i+3);
                    j = ["ge","GT","gt","le","LT","lt","mu","Mu","ne","ni","nu","Nu","or","pi","Pi","Xi","xi"].indexOf(subStr);
                    if (j >= 0) {
                        i += 4;
                        output += ["\u2265",">",">","\u2264","<","<","\u03BC","\u039C","\u2260","\u220B","\u03BD","\u039D","\u2228","\u03C0","\u03A0","\u039E","\u03BE"][j];
                ...
                if (input.charAt(i+4) == ';') {
                    subStr = input.substring(i+1,i+4);
                    j = ["amp","AMP","and","ang","cap","chi","Chi","cup","deg","eta","Eta","eth","ETH","int","loz","lrm","not","Phi","phi","piv","psi","Psi","reg","REG","rho","Rho","rlm","shy","sim","sub","sum","sup","Tau","tau","uml","yen","zwj"].indexOf(subStr);
                    if (j >= 0) {
                        i += 5;
                        output += ["&","&","\u2227","\u2220","\u2229","\u03C7","\u03A7","\u222A","\xB0","\u03B7","\u0397","\xF0","\xD0","\u222B","\u25CA","\u200E","\xAC","\u03A6","\u03C6","\u03D6","\u03C8","\u03A8","\xAE","\xAE","\u03C1","\u03A1","\u200F","\xAD","\u223C","\u2282","\u2211","\u2283","\u03A4","\u03C4","\xA8","\xA5","\u200D"][j];
                ...
                if (input.charAt(i+5) == ';') {
                    subStr = input.substring(i+1,i+5);
                    j = ["apos","auml","Auml","beta","Beta","bull","cent","circ","cong","copy","COPY","dArr","darr","emsp","ensp","Euml","euml","euro","fnof","hArr","harr","iota","Iota","isin","Iuml","iuml","lang","larr","lArr","macr","nbsp","nsub","ordf","ordm","ouml","Ouml","para","part","perp","prod","prop","quot","QUOT","rang","rArr","rarr","real","sdot","sect","sube","sup1","sup2","sup3","supe","uArr","uarr","uuml","Uuml","yuml","Yuml","zeta","Zeta","zwnj"].indexOf(subStr);
                    if (j >= 0) {
                        i += 6;
                        output += ["'","\xE4","\xC4","\u03B2","\u0392","\u2022","\xA2","\u02C6","\u2245","\xA9","\xA9","\u21D3","\u2193","\u2003","\u2002","\xCB","\xEB","\u20AC","\u0192","\u21D4","\u2194","\u03B9","\u0399","\u2208","\xCF","\xEF","\u27E8","\u2190","\u21D0","\xAF","\xA0","\u2284","\xAA","\xBA","\xF6","\xD6","\xB6","\u2202","\u22A5","\u220F","\u221D","\"","\"","\u27E9","\u21D2","\u2192","\u211C","\u22C5","\xA7","\u2286","\xB9","\xB2","\xB3","\u2287","\u21D1","\u2191","\xFC","\xDC","\xFF","\u0178","\u03B6","\u0396","\u200C"][j];
                ...
                if (input.charAt(i+6) == ';') {
                    subStr = input.substring(i+1,i+6);
                    j = ["Acirc","acirc","acute","aelig","AElig","Alpha","alpha","aring","Aring","asymp","bdquo","cedil","clubs","crarr","delta","Delta","diams","ecirc","Ecirc","empty","equiv","exist","frasl","gamma","Gamma","icirc","Icirc","iexcl","image","infin","kappa","Kappa","laquo","lceil","ldquo","lsquo","mdash","micro","minus","nabla","ndash","notin","ocirc","Ocirc","oelig","OElig","oline","Omega","omega","oplus","pound","prime","Prime","radic","raquo","rceil","rdquo","rsquo","sbquo","sigma","Sigma","szlig","Theta","theta","THORN","thorn","tilde","times","trade","ucirc","Ucirc","upsih"].indexOf(subStr);
                    if (j >= 0) {
                        i += 7;
                        output += ["\xC2","\xE2","\xB4","\xE6","\xC6","\u0391","\u03B1","\xE5","\xC5","\u2248","\u201E","\xB8","\u2663","\u21B5","\u03B4","\u0394","\u2666","\xEA","\xCA","\u2205","\u2261","\u2203","\u2044","\u03B3","\u0393","\xEE","\xCE","\xA1","\u2111","\u221E","\u03BA","\u039A","\xAB","\u2308","\u201C","\u2018","\u2014","\xB5","\u2212","\u2207","\u2013","\u2209","\xF4","\xD4","\u0153","\u0152","\u203E","\u03A9","\u03C9","\u2295","\xA3","\u2032","\u2033","\u221A","\xBB","\u2309","\u201D","\u2019","\u201A","\u03C3","\u03A3","\xDF","\u0398","\u03B8","\xDE","\xFE","\u02DC","\xD7","\u2122","\xFB","\xDB","\u03D2"][j];
                ...
                if (input.charAt(i+7) == ';') {
                    subStr = input.substring(i+1,i+7);
                    j = ["Aacute","aacute","Agrave","agrave","Atilde","atilde","brvbar","Ccedil","ccedil","curren","Dagger","dagger","divide","Eacute","eacute","egrave","Egrave","forall","frac12","frac14","frac34","hearts","hellip","iacute","Iacute","igrave","Igrave","iquest","Lambda","lambda","lfloor","lowast","lsaquo","middot","ntilde","Ntilde","oacute","Oacute","ograve","Ograve","oslash","Oslash","Otilde","otilde","otimes","permil","plusmn","rfloor","rsaquo","scaron","Scaron","sigmaf","spades","there4","thinsp","Uacute","uacute","ugrave","Ugrave","weierp","Yacute","yacute"].indexOf(subStr);
                    if (j >= 0) {
                        i += 8;
                        output += ["\xC1","\xE1","\xC0","\xE0","\xC3","\xE3","\xA6","\xC7","\xE7","\xA4","\u2021","\u2020","\xF7","\xC9","\xE9","\xE8","\xC8","\u2200","\xBD","\xBC","\xBE","\u2665","\u2026","\xED","\xCD","\xEC","\xCC","\xBF","\u039B","\u03BB","\u230A","\u2217","\u2039","\xB7","\xF1","\xD1","\xF3","\xD3","\xF2","\xD2","\xF8","\xD8","\xD5","\xF5","\u2297","\u2030","\xB1","\u230B","\u203A","\u0161","\u0160","\u03C2","\u2660","\u2234","\u2009","\xDA","\xFA","\xF9","\xD9","\u2118","\xDD","\xFD"][j];
                ...
                if (input.charAt(i+8) == ';') {
                    subStr = input.substring(i+1,i+8);
                    j = ["alefsym","Epsilon","epsilon","omicron","Omicron","upsilon","Upsilon"].indexOf(subStr);
                    if (j >= 0) {
                        i += 9;
                        output += ["\u2135","\u0395","\u03B5","\u03BF","\u039F","\u03C5","\u03A5"][j];
                ...
                if (input.charAt(i+9) == ';') {
                    subStr = input.substring(i+1,i+9);
                    if (subStr == "thetasym") {
                        i += 10;
                        output += "\u03D1";
                        continue;
        ...
        if (char == '&') {
            if (i + 5 < len) {
                ...
                if (input.charAt(i+10) == ';') {
                    subStr = input.substring(i+1,i+10);
                    j = ["alefsy","angrtv","angzar","asympe","backsi","becaus","Becaus","bempty","betwee","bigcir","bigodo","bigsta","bnequi","boxplu","Cayley","Cconin","ccupss","Cedill","cempty","cirsci","colone","congdo","cudarr","cudarr","cularr","curarr","dbkaro","ddagge","ddotse","dempty","diamon","Diamon","digamm","dotplu","DownTe","dwangl","Elemen","Epsilo","epsilo","eqcolo","equivD","gesdot","gtques","gtrles","harrci","Implie","intpro","isindo","larrbf","larrsi","lbrksl","lbrksl","ldrdha","LeftTe","lesdot","lessdo","lessgt","lesssi","lotime","lozeng","ltques","luruha","maltes","minusd","nappro","natura","nearro","NewLin","nexist","NoBrea","notinv","notinv","notinv","NotLes","notniv","notniv","notniv","npolin","nprece","nsqsub","nsqsup","nsubse","nsucce","nsupse","nvinfi","nvltri","nvrtri","nwarro","olcros","Omicro","omicro","ordero","orslop","OverBa","perten","planck","plusci","plussi","plustw","precsi","Produc","quatin","queste","rarrbf","rarrsi","rbrksl","rbrksl","rdldha","realin","rotime","ruluha","searro","simplu","simrar","subedo","submul","subplu","subrar","succsi","supdsu","supedo","suphso","suphsu","suplar","supmul","supplu","swarro","topfor","triplu","tritim","uparro","UpArro","Uparro","Upsilo","upsilo","uwangl","vzigza","zigrar"].indexOf(subStr);
                    if (j >= 0) {
                        i += 11;
                        output += ["\u2135","\u22BE","\u237C","\u224D","\u223D","\u2235","\u2235","\u29B0","\u226C","\u25EF","\u2A00","\u2605","\u2261\u20E5","\u229E","\u212D","\u2230","\u2A50","\xB8","\u29B2","\u29C2","\u2254","\u2A6D","\u2938","\u2935","\u293D","\u293C","\u290F","\u2021","\u2A77","\u29B1","\u22C4","\u22C4","\u03DD","\u2214","\u22A4","\u29A6","\u2208","\u0395","\u03B5","\u2255","\u2A78","\u2A82","\u2A7C","\u2277","\u2948","\u21D2","\u2A3C","\u22F5","\u291F","\u2973","\u298F","\u298D","\u2967","\u22A3","\u2A81","\u22D6","\u2276","\u2272","\u2A34","\u25CA","\u2A7B","\u2966","\u2720","\u2A2A","\u2249","\u266E","\u2197","\x0A","\u2204","\u2060","\u2209","\u22F7","\u22F6","\u226E","\u220C","\u22FE","\u22FD","\u2A14","\u2AAF\u0338","\u22E2","\u22E3","\u2282\u20D2","\u2AB0\u0338","\u2283\u20D2","\u29DE","\u22B4\u20D2","\u22B5\u20D2","\u2196","\u29BB","\u039F","\u03BF","\u2134","\u2A57","\u203E","\u2031","\u210E","\u2A22","\u2A26","\u2A27","\u227E","\u220F","\u2A16","\u225F","\u2920","\u2974","\u298E","\u2990","\u2969","\u211B","\u2A35","\u2968","\u2198","\u2A24","\u2972","\u2AC3","\u2AC1","\u2ABF","\u2979","\u227F","\u2AD8","\u2AC4","\u27C9","\u2AD7","\u297B","\u2AC2","\u2AC0","\u2199","\u2ADA","\u2A39","\u2A3B","\u2191","\u2191","\u21D1","\u03A5","\u03C5","\u29A7","\u299A","\u21DD"][j];
                ...
                if (input.charAt(i+11) == ';') {
                    subStr = input.substring(i+1,i+11);
                    j = ["andslop","angmsda","angmsda","angmsda","angmsda","angmsda","angmsda","angmsda","angmsda","angrtvb","approxe","awconin","backcon","barwedg","bbrktbr","bigoplu","bigsqcu","biguplu","bigwedg","boxminu","boxtime","bsolhsu","capbrcu","circled","circled","cirfnin","clubsui","cupbrca","curlyve","cwconin","DDotrah","doteqdo","DotEqua","dotminu","drbkaro","dzigrar","elinter","emptyse","eqvpars","fpartin","geqslan","gesdoto","gnappro","hksearo","hkswaro","imaglin","imagpar","infinti","integer","Integra","interca","intlarh","laempty","ldrusha","leqslan","lesdoto","LessLes","llcorne","lnappro","lrcorne","lurdsha","mapstou","multima","natural","ncongdo","NotEqua","notindo","NotTild","otimesa","paralle","Partial","plusaci","pointin","Precede","precneq","precnsi","profala","proflin","profsur","raempty","realpar","RightTe","rppolin","rtriltr","scpolin","setminu","shortmi","smepars","sqsubse","sqsupse","subsete","Succeed","succneq","succnsi","SuchTha","Superse","supsete","thetasy","thicksi","timesba","triangl","triminu","trpeziu","Uarroci","ulcorne","UnderBa","urcorne","varkapp","varsigm","varthet"].indexOf(subStr);
                    if (j >= 0) {
                        i += 12;
                        output += ["\u2A58","\u29A8","\u29A9","\u29AA","\u29AB","\u29AC","\u29AD","\u29AE","\u29AF","\u299D","\u224A","\u2233","\u224C","\u2305","\u23B6","\u2A01","\u2A06","\u2A04","\u22C0","\u229F","\u22A0","\u27C8","\u2A49","\xAE","\u24C8","\u2A10","\u2663","\u2A48","\u22CE","\u2232","\u2911","\u2251","\u2250","\u2238","\u2910","\u27FF","\u23E7","\u2205","\u29E5","\u2A0D","\u2A7E","\u2A84","\u2A8A","\u2925","\u2926","\u2110","\u2111","\u29DD","\u2124","\u222B","\u22BA","\u2A17","\u29B4","\u294B","\u2A7D","\u2A83","\u2AA1","\u231E","\u2A89","\u231F","\u294A","\u21A5","\u22B8","\u2115","\u2A6D\u0338","\u2260","\u22F5\u0338","\u2241","\u2A36","\u2225","\u2202","\u2A23","\u2A15","\u227A","\u2AB5","\u22E8","\u232E","\u2312","\u2313","\u29B3","\u211C","\u22A2","\u2A12","\u29CE","\u2A13","\u2216","\u2223","\u29E4","\u228F","\u2290","\u2286","\u227B","\u2AB6","\u22E9","\u220B","\u2283","\u2287","\u03D1","\u223C","\u2A31","\u25B5","\u2A3A","\u23E2","\u2949","\u231C","_","\u231D","\u03F0","\u03C2","\u03D1"][j];
                ...
                if (input.charAt(i+12) == ';') {
                    subStr = input.substring(i+1,i+12);
                    j = ["backprim","backsime","Backslas","bigotime","centerdo","CenterDo","checkmar","CircleDo","complexe","Congruen","Coproduc","dotsquar","DoubleDo","downarro","DownArro","Downarro","DownBrev","gtrappro","gtreqles","gvertneq","heartsui","HumpEqua","leftarro","LeftArro","Leftarro","LeftFloo","lesseqgt","LessTild","lvertneq","Mellintr","MinusPlu","ngeqslan","nleqslan","NotCupCa","NotExist","NotSubse","nparalle","nshortmi","nsubsete","nsupsete","OverBrac","pitchfor","PlusMinu","rational","spadesui","subseteq","subsetne","supseteq","supsetne","therefor","Therefor","ThinSpac","triangle","TripleDo","UnionPlu","varpropt"].indexOf(subStr);
                    if (j >= 0) {
                        i += 13;
                        output += ["\u2035","\u22CD","\u2216","\u2A02","\xB7","\xB7","\u2713","\u2299","\u2102","\u2261","\u2210","\u22A1","\xA8","\u2193","\u2193","\u21D3","\u0311","\u2A86","\u22DB","\u2269\uFE00","\u2665","\u224F","\u2190","\u2190","\u21D0","\u230A","\u22DA","\u2272","\u2268\uFE00","\u2133","\u2213","\u2A7E\u0338","\u2A7D\u0338","\u226D","\u2204","\u2282\u20D2","\u2226","\u2224","\u2288","\u2289","\u23DE","\u22D4","\xB1","\u211A","\u2660","\u2AC5","\u228A","\u2AC6","\u228B","\u2234","\u2234","\u2009","\u225C","\u20DB","\u228E","\u221D"][j];
                ...
                if (input.charAt(i+13) == ';') {
                    subStr = input.substring(i+1,i+13);
                    j = ["Bernoulli","circledas","CirclePlu","complemen","curlywedg","eqslantgt","EqualTild","Fouriertr","gtreqqles","Imaginary","Laplacetr","LeftVecto","lessappro","lesseqqgt","Lleftarro","lmoustach","longmapst","mapstodow","mapstolef","nleftarro","nLeftarro","NotElemen","NotGreate","nsubseteq","nsupseteq","precappro","Proportio","rightarro","RightArro","Rightarro","RightFloo","rmoustach","sqsubsete","sqsupsete","subsetneq","succappro","supsetneq","ThickSpac","TildeEqua","TildeTild","UnderBrac","UpArrowBa","UpTeeArro","upuparrow","varepsilo","varnothin"].indexOf(subStr);
                    if (j >= 0) {
                        i += 14;
                        output += ["\u212C","\u229B","\u2295","\u2201","\u22CF","\u2A96","\u2242","\u2131","\u2A8C","\u2148","\u2112","\u21BC","\u2A85","\u2A8B","\u21DA","\u23B0","\u27FC","\u21A7","\u21A4","\u219A","\u21CD","\u2209","\u226F","\u2AC5\u0338","\u2AC6\u0338","\u2AB7","\u2237","\u2192","\u2192","\u21D2","\u230B","\u23B1","\u2291","\u2292","\u2ACB","\u2AB8","\u2ACC","\u205F\u200A","\u2243","\u2248","\u23DF","\u2912","\u21A5","\u21C8","\u03F5","\u2205"][j];
                ...
                if (input.charAt(i+14) == ';') {
                    subStr = input.substring(i+1,i+14);
                    j = ["backepsilo","blacksquar","circledcir","circleddas","CircleMinu","CircleTime","curlyeqpre","curlyeqsuc","diamondsui","eqslantles","Equilibriu","expectatio","GreaterLes","LeftCeilin","LessGreate","MediumSpac","NotLessLes","NotPrecede","NotSucceed","NotSuperse","nrightarro","nRightarro","OverBracke","preccurlye","precnappro","quaternion","RightVecto","Rrightarro","RuleDelaye","SmallCircl","SquareUnio","straightph","SubsetEqua","succcurlye","succnappro","thickappro","updownarro","UpDownArro","Updownarro","VerticalBa"].indexOf(subStr);
                    if (j >= 0) {
                        i += 15;
                        output += ["\u03F6","\u25AA","\u229A","\u229D","\u2296","\u2297","\u22DE","\u22DF","\u2666","\u2A95","\u21CC","\u2130","\u2277","\u2308","\u2276","\u205F","\u226A\u0338","\u2280","\u2281","\u2283\u20D2","\u219B","\u21CF","\u23B4","\u227C","\u2AB9","\u210D","\u21C0","\u21DB","\u29F4","\u2218","\u2294","\u03D5","\u2286","\u227D","\u2ABA","\u2248","\u2195","\u2195","\u21D5","\u2223"][j];
                ...
                if (input.charAt(i+15) == ';') {
                    subStr = input.substring(i+1,i+15);
                    j = ["blacklozeng","DownArrowBa","DownTeeArro","exponential","Exponential","GreaterEqua","GreaterTild","HilbertSpac","HumpDownHum","Intersectio","LeftArrowBa","LeftTeeArro","LeftTriangl","LeftUpVecto","NotCongruen","NotHumpEqua","NotLessEqua","NotLessTild","Proportiona","RightCeilin","risingdotse","RoundImplie","ShortUpArro","SquareSubse","triangledow","trianglelef","UnderBracke","varsubsetne","varsupsetne","VerticalLin"].indexOf(subStr);
                    if (j >= 0) {
                        i += 16;
                        output += ["\u29EB","\u2913","\u21A7","\u2147","\u2147","\u2265","\u2273","\u210B","\u224E","\u22C2","\u21E4","\u21A4","\u22B2","\u21BF","\u2262","\u224F\u0338","\u2270","\u2274","\u221D","\u2309","\u2253","\u2970","\u2191","\u228F","\u25BF","\u25C3","\u23B5","\u228A\uFE00","\u228B\uFE00","|"][j];
                ...
                if (input.charAt(i+16) == ';') {
                    subStr = input.substring(i+1,i+16);
                    j = ["ApplyFunctio","bigtriangleu","blacktriangl","Differential","divideontime","DoubleLeftTe","DoubleUpArro","fallingdotse","hookleftarro","leftarrowtai","leftharpoonu","LeftTeeVecto","LeftVectorBa","LessFullEqua","longleftarro","LongLeftArro","Longleftarro","looparrowlef","measuredangl","NotEqualTild","NotTildeEqua","NotTildeTild","ntrianglelef","Poincareplan","PrecedesEqua","PrecedesTild","RightArrowBa","RightTeeArro","RightTriangl","RightUpVecto","shortparalle","smallsetminu","SucceedsEqua","SucceedsTild","SupersetEqua","trianglerigh","UpEquilibriu","upharpoonlef","varsubsetneq","varsupsetneq","VerticalTild","VeryThinSpac"].indexOf(subStr);
                    if (j >= 0) {
                        i += 17;
                        output += ["\u2061","\u25B3","\u25B4","\u2146","\u22C7","\u2AE4","\u21D1","\u2252","\u21A9","\u21A2","\u21BC","\u295A","\u2952","\u2266","\u27F5","\u27F5","\u27F8","\u21AB","\u2221","\u2242\u0338","\u2244","\u2249","\u22EA","\u210C","\u2AAF","\u227E","\u21E5","\u21A6","\u22B3","\u21BE","\u2225","\u2216","\u2AB0","\u227F","\u2287","\u25B9","\u296E","\u21BF","\u2ACB\uFE00","\u2ACC\uFE00","\u2240","\u200A"][j];
                ...
                if (input.charAt(i+17) == ';') {
                    subStr = input.substring(i+1,i+17);
                    j = ["curvearrowlef","DiacriticalDo","doublebarwedg","DoubleRightTe","downdownarrow","DownLeftVecto","GreaterGreate","hookrightarro","HorizontalLin","InvisibleComm","InvisibleTime","LeftDownVecto","leftleftarrow","leftrightarro","LeftRightArro","Leftrightarro","leftthreetime","LessSlantEqua","longrightarro","LongRightArro","Longrightarro","looparrowrigh","LowerLeftArro","NestedLessLes","NotGreaterLes","NotLessGreate","NotSubsetEqua","NotVerticalBa","nshortparalle","ntrianglerigh","OpenCurlyQuot","ReverseElemen","rightarrowtai","rightharpoonu","RightTeeVecto","RightVectorBa","ShortDownArro","ShortLeftArro","SquareSuperse","TildeFullEqua","trianglelefte","upharpoonrigh","UpperLeftArro","ZeroWidthSpac"].indexOf(subStr);
                    if (j >= 0) {
                        i += 18;
                        output += ["\u21B6","\u02D9","\u2306","\u22A8","\u21CA","\u21BD","\u2AA2","\u21AA","\u2500","\u2063","\u2062","\u21C3","\u21C7","\u2194","\u2194","\u21D4","\u22CB","\u2A7D","\u27F6","\u27F6","\u27F9","\u21AC","\u2199","\u226A","\u2279","\u2278","\u2288","\u2224","\u2226","\u22EB","\u2018","\u220B","\u21A3","\u21C0","\u295B","\u2953","\u2193","\u2190","\u2290","\u2245","\u22B4","\u21BE","\u2196","\u200B"][j];
                ...
                if (input.charAt(i+18) == ';') {
                    subStr = input.substring(i+1,i+18);
                    j = ["bigtriangledow","circlearrowlef","CloseCurlyQuot","ContourIntegra","curvearrowrigh","DoubleDownArro","DoubleLeftArro","downharpoonlef","DownRightVecto","leftharpoondow","leftrightarrow","LeftRightVecto","LeftTriangleBa","LeftUpTeeVecto","LeftUpVectorBa","LowerRightArro","nleftrightarro","nLeftrightarro","NotGreaterEqua","NotGreaterTild","NotHumpDownHum","NotLeftTriangl","NotSquareSubse","ntrianglelefte","OverParenthesi","RightDownVecto","rightleftarrow","rightsquigarro","rightthreetime","ShortRightArro","straightepsilo","trianglerighte","UpperRightArro","vartrianglelef"].indexOf(subStr);
                    if (j >= 0) {
                        i += 19;
                        output += ["\u25BD","\u21BA","\u2019","\u222E","\u21B7","\u21D3","\u21D0","\u21C3","\u21C1","\u21BD","\u21C6","\u294E","\u29CF","\u2960","\u2958","\u2198","\u21AE","\u21CE","\u2271","\u2275","\u224E\u0338","\u22EA","\u228F\u0338","\u22EC","\u23DC","\u21C2","\u21C4","\u219D","\u22CC","\u2192","\u03F5","\u22B5","\u2197","\u22B2"][j];
                ...
                if (input.charAt(i+19) == ';') {
                    subStr = input.substring(i+1,i+19);
                    j = ["circlearrowrigh","DiacriticalAcut","DiacriticalGrav","DiacriticalTild","DoubleRightArro","DownArrowUpArro","downharpoonrigh","EmptySmallSquar","GreaterEqualLes","GreaterFullEqua","LeftAngleBracke","LeftUpDownVecto","LessEqualGreate","NonBreakingSpac","NotPrecedesEqua","NotRightTriangl","NotSucceedsEqua","NotSucceedsTild","NotSupersetEqua","ntrianglerighte","rightharpoondow","rightrightarrow","RightTriangleBa","RightUpTeeVecto","RightUpVectorBa","twoheadleftarro","UnderParenthesi","UpArrowDownArro","vartrianglerigh"].indexOf(subStr);
                    if (j >= 0) {
                        i += 20;
                        output += ["\u21BB","\xB4","`","\u02DC","\u21D2","\u21F5","\u21C2","\u25FB","\u22DB","\u2267","\u27E8","\u2951","\u22DA","\xA0","\u2AAF\u0338","\u22EB","\u2AB0\u0338","\u227F\u0338","\u2289","\u22ED","\u21C1","\u21C9","\u29D0","\u295C","\u2954","\u219E","\u23DD","\u21C5","\u22B3"][j];
                ...
                if (input.charAt(i+20) == ';') {
                    subStr = input.substring(i+1,i+20);
                    j = ["blacktriangledow","blacktrianglelef","DoubleUpDownArro","DoubleVerticalBa","DownLeftTeeVecto","DownLeftVectorBa","FilledSmallSquar","GreaterSlantEqua","LeftDoubleBracke","LeftDownTeeVecto","LeftDownVectorBa","leftrightharpoon","LeftTriangleEqua","NegativeThinSpac","NotGreaterGreate","NotLessSlantEqua","NotNestedLessLes","NotReverseElemen","NotSquareSuperse","NotTildeFullEqua","RightAngleBracke","rightleftharpoon","RightUpDownVecto","SquareSubsetEqua","twoheadrightarro","VerticalSeparato"].indexOf(subStr);
                    if (j >= 0) {
                        i += 21;
                        output += ["\u25BE","\u25C2","\u21D5","\u2225","\u295E","\u2956","\u25FC","\u2A7E","\u27E6","\u2961","\u2959","\u21CB","\u22B4","\u200B","\u226B\u0338","\u2A7D\u0338","\u2AA1\u0338","\u220C","\u2290\u0338","\u2247","\u27E9","\u21CC","\u294F","\u2291","\u21A0","\u2758"][j];
                ...
                if (input.charAt(i+21) == ';') {
                    subStr = input.substring(i+1,i+21);
                    j = ["blacktrianglerigh","DownRightTeeVecto","DownRightVectorBa","longleftrightarro","LongLeftRightArro","Longleftrightarro","NegativeThickSpac","NotLeftTriangleBa","PrecedesSlantEqua","ReverseEquilibriu","RightDoubleBracke","RightDownTeeVecto","RightDownVectorBa","RightTriangleEqua","SquareIntersectio","SucceedsSlantEqua"].indexOf(subStr);
                    if (j >= 0) {
                        i += 22;
                        output += ["\u25B8","\u295F","\u2957","\u27F7","\u27F7","\u27FA","\u200B","\u29CF\u0338","\u227C","\u21CB","\u27E7","\u295D","\u2955","\u22B5","\u2293","\u227D"][j];
                ...
                if (input.charAt(i+22) == ';') {
                    subStr = input.substring(i+1,i+22);
                    j = ["DoubleLongLeftArro","DownLeftRightVecto","LeftArrowRightArro","leftrightsquigarro","NegativeMediumSpac","NotGreaterFullEqua","NotRightTriangleBa","RightArrowLeftArro","SquareSupersetEqua"].indexOf(subStr);
                    if (j >= 0) {
                        i += 23;
                        output += ["\u27F8","\u2950","\u21C6","\u21AD","\u200B","\u2267\u0338","\u29D0\u0338","\u21C4","\u2292"][j];
                ...
                if (input.charAt(i+23) == ';') {
                    subStr = input.substring(i+1,i+23);
                    j = ["CapitalDifferential","DoubleLeftRightArro","DoubleLongRightArro","EmptyVerySmallSquar","NestedGreaterGreate","NotDoubleVerticalBa","NotGreaterSlantEqua","NotLeftTriangleEqua","NotSquareSubsetEqua","OpenCurlyDoubleQuot","ReverseUpEquilibriu"].indexOf(subStr);
                    if (j >= 0) {
                        i += 24;
                        output += ["\u2145","\u21D4","\u27F9","\u25AB","\u226B","\u2226","\u2A7E\u0338","\u22EC","\u22E2","\u201C","\u296F"][j];
                ...
                if (input.charAt(i+24) == ';') {
                    subStr = input.substring(i+1,i+24);
                    j = ["CloseCurlyDoubleQuot","DoubleContourIntegra","FilledVerySmallSquar","NegativeVeryThinSpac","NotPrecedesSlantEqua","NotRightTriangleEqua","NotSucceedsSlantEqua"].indexOf(subStr);
                    if (j >= 0) {
                        i += 25;
                        output += ["\u201D","\u222F","\u25AA","\u200B","\u22E0","\u22ED","\u22E1"][j];
                ...
                if (input.charAt(i+25) == ';') {
                    subStr = input.substring(i+1,i+25);
                    if (subStr == "DiacriticalDoubleAcut") {
                        i += 26;
                        output += "\u02DD";
                        continue;
                ...
                if (input.charAt(i+26) == ';') {
                    subStr = input.substring(i+1,i+26);
                    if (subStr == "NotNestedGreaterGreate") {
                        i += 27;
                        output += "\u2AA2\u0338";
                        continue;
                ...
                if (input.charAt(i+27) == ';') {
                    subStr = input.substring(i+1,i+27);
                    if (subStr == "ClockwiseContourIntegra") {
                        i += 28;
                        output += "\u2232";
                        continue;
                ...
                if (input.charAt(i+34) == ';') {
                    subStr = input.substring(i+1,i+34);
                    if (subStr == "CounterClockwiseContourIntegra") {
                        i += 35;
                        output += "\u2233";
                        continue;
